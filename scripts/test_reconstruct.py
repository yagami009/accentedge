#!/usr/bin/env python3
"""Minimal FACodec reconstruction test using upstream FAcodec.

Runs the exact upstream FAcodec reconstruct.py pattern:
  z = model.encoder(wav)
  z, quantized, commitment_loss, codebook_loss, timbre = model.quantizer(z, wav, n_c=2)
  full_pred_wave = model.decoder(z)

Gate: mel L1 < 0.15 (acceptable for high-compression codec)

Run on Colab:
  colab run scripts/test_reconstruct.py --gpu T4
"""
import subprocess, sys, os, types, warnings

warnings.simplefilter("ignore")

# Mock audiotools before ANY imports (prevents metaclass conflict in DAC)
...
print("\n=== Loading FAcodec (upstream pattern) ===")
...
# Round-trip test (exact upstream reconstruct.py pattern)
...
for fname in test_samples:
    ...
    print(f"  {os.path.basename(fname)}: Mel L1={mel_l1:.4f}")
