"""Fake leak model — accesses future input (should fail causality test)."""

from __future__ import annotations

import numpy as np

from accentedge_lab.models.interfaces import (
    CandidateMetadata,
    StreamingCandidate,
    StreamingResult,
    StreamingSession,
)


class FakeLeakModel:
    metadata = CandidateMetadata(
        architecture_id="fake_leak",
        version="0.0.1",
        input_sample_rate=16000,
        frame_ms=10.0,
        preferred_chunk_ms=80,
        required_lookahead_ms=0,
        left_context_ms=0,
    )

    def prepare(self, device: str, precision: str) -> None:
        pass

    def create_session(self, config: dict) -> StreamingSession:
        return StreamingSession(
            session_id="leak_session",
            state={"buffer": np.zeros(0, dtype=np.float32)},
        )

    def process_chunk(
        self,
        session: StreamingSession,
        audio_chunk: np.ndarray,
        sample_rate: int,
    ) -> StreamingResult:
        session.state["buffer"] = np.concatenate(
            [session.state["buffer"], audio_chunk]
        )
        # Output depends on global buffer (leaks future via cumulative stats)
        out = np.full(
            len(audio_chunk), np.max(session.state["buffer"]), dtype=np.float32
        )
        session.samples_processed += len(audio_chunk)
        return StreamingResult(
            audio=out,
            sample_rate=sample_rate,
            input_start_sample=session.samples_processed - len(audio_chunk),
            input_end_sample=session.samples_processed,
            output_start_sample=session.samples_processed - len(audio_chunk),
            output_end_sample=session.samples_processed,
        )

    def flush(self, session: StreamingSession) -> list[StreamingResult]:
        return []

    def reset(self, session: StreamingSession) -> None:
        session.state["buffer"] = np.zeros(0, dtype=np.float32)
        session.samples_processed = 0

    def close(self) -> None:
        pass
